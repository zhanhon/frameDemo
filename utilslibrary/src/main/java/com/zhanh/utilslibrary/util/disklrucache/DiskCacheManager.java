...
import com.zhanh.utilslibrary.util.AppUtils;
import com.zhanh.utilslibrary.util.MD5Utils;
...
public class DiskCacheManager {
    ...
    public static DiskCacheManager getDiskCacheManager() {
        if (instance == null){
            synchronized (DiskCacheManager.class){
                if (instance == null) {
                    instance = new DiskCacheManager(BaseAppContext.getInstance(),"net_data");
                }
            }
        }
        return instance;
    ...
    private DiskCacheManager(Context context, String uniqueName) {
        try {
            //先关闭已有的缓存
            ...
            mDiskLruCache = DiskLruCache.open(cacheFile, AppUtils.getVersionCode(context), 1, 10 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private DiskLruCache.Editor edit(String key) throws IOException {
        key = MD5Utils.stringToMD5(key); //存取的 key
        if (mDiskLruCache != null) {
